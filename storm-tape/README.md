...
This repository contains the StoRM implementation of the WLCG Tape REST API defined in
https://docs.google.com/document/d/1Zx_H5dRkQRfju3xIYZ2WgjKoOvmLtsafP2pKGpHqcfY/edit?usp=sharing.

## How to build

The dependencies are managed with [vcpkg](https://vcpkg.io/), which needs to be
installed first. The bare minimum is the following:
...
The dependencies are specified in a [manifest](vcpkg.json) file and are built
from source, if necessary, during the build of StoRM Tape. The binary artifacts
are kept in a subdirectory under ${VCPKG_ROOT}.

To build StoRM Tape we recommend to use the Ninja Multi-Config generator with
CMake:
...
```shell
$ build/Debug/storm-tape-debug # or build/Release/storm-tape
...
By default, `storm-tape` reads the `storm-tape.conf` configuration file from the
current directory. A minimal configuration, usable with the examples shown
below, can be:
...
As an alternative, it is possible to run StoRM Tape via Docker, using the following command:

```
docker run -it -p 8080:8080 -v <local-dir>:/storm-tape baltig.infn.it:4567/cnafsd/storm-tape:<tag>
```

`<local-dir>` is a directory that, for example, contains the configuration file.
For the list of available tags, see
https://baltig.infn.it/cnafsd/storm-tape/container_registry/375
...
And again, requesting the archive information about the same files, now gives
...
It is possibile to build binaries with coverage in order to produce a detailed
HTML report.

To build with coverage use

```shell
cmake --build build --config Coverage
```

To produce the coverage report, run one or more executables, followed by
`scripts/cov-report.sh`, which will generate the HTML report.

For example, to produce the coverage report for the unit tests:

```shell
build/tests/Coverage/all.t
scripts/cov-report.sh
```

The full report will be saved to `build/report` (hardcoded).
