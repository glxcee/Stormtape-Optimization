...
TEST_CASE("Update timestamps")
{
  ...
  storm::StageRequest completed_stage{completed_files};
  completed_stage.update_timestamps();

  CHECK_EQ(completed_stage.started_at, now);
  CHECK_EQ(completed_stage.completed_at, now + 50);
