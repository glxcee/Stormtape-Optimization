...
{
  // Create Stage table
  m_sql << "CREATE TABLE IF NOT EXISTS Stage ("
           "id           TEXT   PRIMARY KEY,"
           "created_at   BIGINT NOT NULL,"
           "started_at   BIGINT NOT NULL,"
           "completed_at BIGINT NOT NULL);";
  // Create File table
  m_sql << "CREATE TABLE IF NOT EXISTS File ("
...
bool SociDatabase::insert(StageId const& id, StageRequest const& stage)
{
  ...
  try {
    ...
    // Insert stage
    m_sql << "INSERT INTO Stage VALUES (:id, :created_at, :started_at, "
             ":completed_at);",
        soci::use(s_entity);
    ...
    std::for_each(files.begin(), files.end(), [&](auto const& f) {
      FileEntity const entity{id,           f.logical_path, f.physical_path,
                              ...
                              f.finished_at};
      m_sql << "INSERT INTO File VALUES (:stage_id, :logical_path, "
...
std::optional<StageRequest> SociDatabase::find(StageId const& id) const
{
  ...
  m_sql << "SELECT * FROM Stage WHERE id = :id;", soci::into(s_entity),
      soci::use(id);
